...
export class AssignmentController {
  ...
  public async endAssignment(assignmentID: number, res: Response) {
    let assignment = await assignmentService.getAssignmentById(assignmentID);
        ...
        // if assignment end date has passed, throw error
        const currDate = new Date();
        if (assignment.endOfAssignment && assignment.endOfAssignment < currDate){
            ConsoleLogger.logWarning("Assignment end date has already passed");
  ...
  public async getAssignment(assignmentID: number, res: Response) {
     ...
     */
    public async createAssignment(data:Omit<Assignment, "id">, req:Request, res:Response) { 
        ...
        if (asset === null) {
            ...
            return res.sendStatus(404);
        } else if(asset.custodian !== currUser.id){
            ConsoleLogger.logWarning("User is not the custodian of this asset");
